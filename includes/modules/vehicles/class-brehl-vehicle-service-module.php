...
final class Brehl_Vehicle_Service_Module {
    ...
    public function request_panel(): string {
        ...
        wp_enqueue_style('brehl-intranet'); wp_enqueue_style('my-brehl-system'); wp_enqueue_style('brehl-intranet-vehicle-damage'); wp_enqueue_script('brehl-intranet-vacation');
        global $wpdb;
    ...
    public function management_panel(): string {
        ...
        wp_enqueue_style('brehl-intranet'); wp_enqueue_style('my-brehl-system'); wp_enqueue_style('brehl-intranet-vehicle-damage'); wp_enqueue_script('brehl-intranet-vacation');
        global $wpdb;
        $archive=!empty($_GET['service_archive']);
        $condition=$archive?"r.status IN ('completed','rejected')":"r.status NOT IN ('completed','rejected')";
        $per_page=20; $total=(int)$wpdb->get_var("SELECT COUNT(*) FROM {$this->table()} r WHERE {$condition}");
        $page=min(max(1,absint($_GET['service_page']??1)),max(1,(int)ceil($total/$per_page))); $offset=($page-1)*$per_page;
        $items = $wpdb->get_results($wpdb->prepare("SELECT r.*,u.display_name FROM {$this->table()} r LEFT JOIN {$wpdb->users} u ON u.ID=r.user_id WHERE {$condition} ORDER BY FIELD(r.status,'submitted','review','scheduled','workshop','completed','rejected'),r.created_at DESC LIMIT %d OFFSET %d",$per_page,$offset));
        $toggle=$archive?remove_query_arg(array('service_archive','service_page')):add_query_arg('service_archive','1',remove_query_arg('service_page'));
        $result = sanitize_key($_GET['vehicle_service_management'] ?? '');
        ob_start(); ?>
        <section class="mbs-service-management"><div class="mbs-card"><div class="mbs-card-head"><div><span class="mbs-kicker"><?php esc_html_e('Fuhrpark', 'brehl-intranet'); ?></span><h3><?php echo $archive?esc_html__('Servicearchiv','brehl-intranet'):esc_html__('Serviceanfragen verwalten', 'brehl-intranet'); ?></h3></div><div class="mbs-vehicle-management-tools"><span class="mbs-count"><?php echo esc_html(sprintf(_n('%d Vorgang','%d Vorgänge',$total,'brehl-intranet'),$total)); ?></span><a href="<?php echo esc_url($toggle); ?>"><?php echo $archive?esc_html__('Aktuelle anzeigen','brehl-intranet'):esc_html__('Archiv anzeigen','brehl-intranet'); ?></a></div></div>
        ...
        <div class="mbs-service-management__list"><?php if (!$items) : ?><p class="mbs-empty"><?php echo $archive?esc_html__('Das Servicearchiv ist leer.','brehl-intranet'):esc_html__('Derzeit liegen keine Serviceanfragen vor.', 'brehl-intranet'); ?></p><?php endif; ?>
        <?php foreach ($items as $item) : $selected=(array)json_decode($item->service_types,true); ?><details class="mbs-service-case"><summary><div><strong><?php echo esc_html($item->license_plate); ?></strong><span><?php echo esc_html(($item->display_name ?: __('Unbekannter Mitarbeiter','brehl-intranet')).' · '.number_format_i18n((int)$item->current_mileage).' km · '.wp_date('d.m.Y',strtotime($item->created_at))); ?></span></div><?php if(!$archive&&'submitted'===$item->status): ?><span class="mbs-vehicle-new-badge"><?php esc_html_e('NEU','brehl-intranet'); ?></span><?php endif; ?><span class="mbs-status"><?php echo esc_html($this->status_label($item->status)); ?></span></summary><div class="mbs-service-case__body"><p class="mbs-service-case__types"><?php echo esc_html(implode(' · ',array_map(fn($type)=>$this->service_types()[$type]??$type,$selected))); ?></p><p><?php echo esc_html($item->description); ?></p>
        <?php if(!$archive): ?><form class="mbs-service-management__form" method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>"><input type="hidden" name="action" value="brehl_manage_vehicle_service"><input type="hidden" name="request_id" value="<?php echo esc_attr((string)$item->id); ?>"><?php wp_nonce_field('brehl_manage_vehicle_service_'.$item->id); ?><fieldset class="mbs-vehicle-status-actions"><legend><?php esc_html_e('Status direkt ändern','brehl-intranet'); ?></legend><?php foreach($this->statuses() as $key=>$label): ?><button class="<?php echo $item->status===$key?'is-current':''; ?>" type="submit" name="status" value="<?php echo esc_attr($key); ?>"><?php echo esc_html($label); ?></button><?php endforeach; ?></fieldset><div class="mbs-form-grid">
        ...
        <label class="mbs-form-full"><span><?php esc_html_e('Rückmeldung an den Mitarbeiter','brehl-intranet'); ?></span><textarea name="admin_note" rows="3" placeholder="Termin, Rückfragen oder Abschlussinformation"><?php echo esc_textarea((string)$item->admin_note); ?></textarea></label></div><p class="mbs-vehicle-save-hint"><?php esc_html_e('Der angeklickte Status wird zusammen mit den Angaben gespeichert.','brehl-intranet'); ?></p></form><?php elseif($item->admin_note): ?><p class="mbs-vehicle-admin-note"><strong><?php esc_html_e('Rückmeldung:','brehl-intranet'); ?></strong> <?php echo esc_html($item->admin_note); ?></p><?php endif; ?></div></details><?php endforeach; ?></div><?php echo $this->pagination_html($page,$total,$per_page,'service_page'); ?></div></section>
        <?php return (string)ob_get_clean();
    }
