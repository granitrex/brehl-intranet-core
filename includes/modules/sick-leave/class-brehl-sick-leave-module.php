...
final class Brehl_Sick_Leave_Module {
    ...
    public function management_panel(): string {
        ...
        $this->enqueue();
        global $wpdb;
        $archive=!empty($_GET['sick_archive']);
        $condition=$archive?"status='bestaetigt'":"status='gemeldet'";
        $per_page=20; $total=(int)$wpdb->get_var("SELECT COUNT(*) FROM {$this->table()} WHERE {$condition}");
        $page=min(max(1,absint($_GET['sick_page']??1)),max(1,(int)ceil($total/$per_page)));
        $items=$wpdb->get_results($wpdb->prepare("SELECT id,user_id,start_date,end_date,employee_note,status,hr_note,certificate_name,created_at FROM {$this->table()} WHERE {$condition} ORDER BY created_at DESC LIMIT %d OFFSET %d",$per_page,($page-1)*$per_page));
        $result = sanitize_key($_GET['sick_management'] ?? '');
        $toggle=$archive?remove_query_arg(array('sick_archive','sick_page')):add_query_arg('sick_archive','1',remove_query_arg('sick_page'));
        ob_start(); ?>
        <section class="mbs-workwear-management mbs-absence-management brehl-sick-management"><div class="mbs-card"><div class="mbs-card-head"><div><span class="mbs-kicker"><?php esc_html_e('Abwesenheiten','brehl-intranet'); ?></span><h3><?php echo $archive?esc_html__('Krankmeldungsarchiv','brehl-intranet'):esc_html__('Krankmeldungen','brehl-intranet'); ?></h3></div></div><div class="mbs-workwear-toolbar"><span><?php echo esc_html(sprintf(_n('%d Vorgang','%d Vorgänge',$total,'brehl-intranet'),$total)); ?></span><a class="mbs-workwear-archive-link" href="<?php echo esc_url($toggle); ?>"><?php echo $archive?esc_html__('Aktuelle Meldungen','brehl-intranet'):esc_html__('Archiv anzeigen','brehl-intranet'); ?></a></div>
        ...
        <div class="mbs-workwear-management__list"><?php if (!$items) : ?><p class="mbs-empty"><?php echo $archive?esc_html__('Das Krankmeldungsarchiv ist leer.','brehl-intranet'):esc_html__('Es liegen keine offenen Krankmeldungen vor.','brehl-intranet'); ?></p><?php endif; ?><?php foreach ($items as $item) : $user=get_userdata((int)$item->user_id); ?><details class="mbs-workwear-case mbs-absence-case"><summary><div><strong><?php echo esc_html($user?$user->display_name:__('Unbekannter Mitarbeiter','brehl-intranet')); ?></strong><small><?php echo esc_html($this->period_label($item).' · '.wp_date('d.m.Y',strtotime($item->created_at))); ?></small></div><?php if(!$archive): ?><span class="mbs-workwear-new-badge"><?php esc_html_e('NEU','brehl-intranet'); ?></span><?php endif; ?><span class="mbs-status mbs-status-<?php echo esc_attr($item->status); ?>"><?php echo esc_html($this->status_label($item->status)); ?></span></summary><div class="mbs-workwear-case__body"><?php if($item->employee_note): ?><p class="mbs-workwear-note"><strong><?php esc_html_e('Hinweis:','brehl-intranet'); ?></strong> <?php echo esc_html($item->employee_note); ?></p><?php endif; ?><?php if($item->certificate_name): ?><p><a class="brehl-sick-certificate" href="<?php echo esc_url($this->download_url((int)$item->id)); ?>"><?php esc_html_e('Geschützte Bescheinigung öffnen','brehl-intranet'); ?></a></p><?php endif; ?><?php if(!$archive): ?><form class="mbs-workwear-management__form" method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>"><input type="hidden" name="action" value="brehl_update_sick_leave"><input type="hidden" name="report_id" value="<?php echo esc_attr((string)$item->id); ?>"><input type="hidden" name="redirect_to" value="<?php echo esc_url(remove_query_arg(array('sick_management','vacation_management','people_result','employee_id'))); ?>"><?php wp_nonce_field('brehl_update_sick_leave_'.$item->id); ?><fieldset class="mbs-workwear-status-actions"><legend><?php esc_html_e('Status direkt ändern','brehl-intranet'); ?></legend><?php foreach(array('gemeldet','bestaetigt') as $status): ?><button class="<?php echo $item->status===$status?'is-current':''; ?>" type="submit" name="status" value="<?php echo esc_attr($status); ?>"><?php echo esc_html($this->status_label($status)); ?></button><?php endforeach; ?></fieldset><label class="mbs-form-full"><span><?php esc_html_e('Rückmeldung','brehl-intranet'); ?></span><textarea name="hr_note" rows="2"><?php echo esc_textarea($item->hr_note); ?></textarea></label><p class="mbs-workwear-save-hint"><?php esc_html_e('Der angeklickte Status wird zusammen mit der Rückmeldung gespeichert.','brehl-intranet'); ?></p></form><?php elseif($item->hr_note): ?><p class="mbs-workwear-note"><strong><?php esc_html_e('Rückmeldung:','brehl-intranet'); ?></strong> <?php echo esc_html($item->hr_note); ?></p><?php endif; ?></div></details><?php endforeach; ?></div><?php echo $this->pagination_html($page,$total,$per_page,'sick_page'); ?></div></section>
        <?php return (string) ob_get_clean();
    }
